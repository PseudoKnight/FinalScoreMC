    ...
    'executor': closure(@alias, @sender, @args, @info) {
        ...
        switch(@args[0]) {
            case 'join':
                @game = import('hazards');
                if(!@game) {
                    @game = _hazards_create();
                }
                _hazards_add_player(@sender, @game);
                broadcast(display_name(@sender).color('reset').' joined Hazards.', all_players(@game['world']));
