...
register_command('cluck', array(
	...
	usage: '/cluck [start|top]',
	tabcompleter: _create_tabcompleter(
		array('group.moderator': array('top', 'resetstats'),
			null: array('top'))
	),
	executor: closure(@alias, @sender, @args, @info) {
		if(!@args || @args[0] === 'top') {
			...
			for(@i = 0, @i < array_size(@top), @i++) {
				@score = @top[@i]['score'];
				msg(if(length(@score) < 2, '0').@score.' - '.@top[@i]['name']);
			}
			return(true);

		} else if(@args[0] === 'resetstats') {
		...
		proc _cluck_defaults() {
			...
			return(array(
				...
				chickens: array(), # spawned chicken entity ids
				hit: 0, # num chickens hit
				player: '',
				gameover: 0, # last round if gameover
				...
				target: null, # target block location
			));
		}
		...
		bind('item_pickup', array(id: 'cluckpickup'), array(player: @player), @event) {
			cancel();
		}
		...
		proc _cluck_startround(@cluck) {
			...
			@angleEnabled = @cluck['round'] >= 2;
			@dispsenersEnabled = @cluck['round'] >= 3;
			@babiesEnabled = @cluck['round'] >= 4;
		
			@babyCount = array(@cluck['round'] - 3);
			queue_delay(2000, 'cluck');
			for(@i = @cluck['count'], @i > 0, @i--) {
				queue_delay(350 * rand(6 - integer(@cluck['round'] / 2), 12 - @cluck['round']), 'cluck');
				queue_push(closure(){
					@loc = null;
					@angle = null;
					@offset = null;
					@dispensed = false;
					if(@dispsenersEnabled && rand() < 0.1) {
					...
					@cluck['chickens'][] = spawn_entity('CHICKEN', 1, @loc, closure(@id) {
						set_entity_saves_on_unload(@id, false);
						if(@babiesEnabled && @babyCount[0]
						&& (@babyCount[0] >= @i || !rand(@i))) {
							@babyCount[0]--;
							set_mob_age(@id, -24000);
						...
						if(@dispensed) {
							set_entity_velocity(@id, get_vector(@loc, 0.5));
							make_effect(@loc, 'SMOKE:'.if(@loc[4], 'NORTH', 'SOUTH'));
						} else {
							set_entity_velocity(@id, array(
								x: 0,
								y: 1.1, 
								z: if(@angleEnabled, (@angle - 12.5 * @offset) * (@cluck['round'] / 1000), 0)));
						}
						set_entity_health(@id, 25);
					})[0];
				}, 'cluck');
			}
