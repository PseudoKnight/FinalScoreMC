			...
			case 'updatedb':
				...
				foreach(@key: @arena in @arenas) {
					if(array_index_exists(@arena, 'kit')) {
						foreach(@kit in @arena['kit']) {
							if(@kit) {
								foreach(@slot: @item in @kit) {
									if(@item) {
										try {
											@kit[@slot] = _minify_item(convert_legacy_item(@item));
										} catch(FormatException @ex) {
											array_remove(@kit, @slot);
											msg(@ex['message']);
										}
									}
								}
					...
					if(array_index_exists(@arena, 'itemspawn')) {
						foreach(@i: @spawn in @arena['itemspawn']) {
							try {
								@spawn['item'] = _minify_item(convert_legacy_item(@spawn['item']));
							} catch(FormatException @ex) {
								array_remove(@arena['itemspawn'], @i);
								msg(@ex['message']);
							}
						}
					}
					if(array_index_exists(@arena, 'chestspawn')) {
						foreach(@spawn in @arena['chestspawn']) {
							if(!array_index_exists(@spawn, 'items')) {
								msg(@spawn);
								continue();
							}
							foreach(@slot: @item in @spawn['items']) {
								if(@item) {
									try {
										@spawn['items'][@slot] = _minify_item(convert_legacy_item(@item));
									} catch(FormatException @ex) {
										array_remove(@spawn['items'], @slot);
										msg(@ex['message']);
									}
								}
							}
					...
					if(array_index_exists(@arena, 'powerup')) {
						@arena['powerup'] = data_name(@arena['powerup']);
					}
					if(array_index_exists(@arena, 'denydrop')) {
						if(is_array(@arena['denydrop'])) {
							foreach(@i: @id in @arena['denydrop']) {
								@arena['powerup'][@i] = data_name(@id);
							}
						}
					}
					if(array_index_exists(@arena, 'classes')) {
						@classes = @arena['classes'];
						foreach(@id: @class in @classes) {
							if(array_index_exists(@class, 'selector')) {
								@class['selector'] = _minify_item(convert_legacy_item(@class['selector']));
							}
							if(array_index_exists(@class, 'kit')) {
								foreach(@slot: @item in @class['kit']) {
									if(@item) {
										try {
											@class['kit'][@slot] = _minify_item(convert_legacy_item(@item));
										} catch(FormatException @ex) {
											array_remove(@class['kit'], @slot);
											msg(@ex['message']);
										}
									}
								}
							...
							if(array_index_exists(@class, 'stacklimit')) {
								foreach(@type: @int in @class['stacklimit']) {
									array_remove(@class['stacklimit'], @type);
									@class['stacklimit'][data_name(@type)] = @int;
								}
							}
						}
					}
